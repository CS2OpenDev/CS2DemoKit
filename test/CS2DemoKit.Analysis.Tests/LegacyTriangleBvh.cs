...
// A FROZEN, VERBATIM copy of TriangleBvh as it stood before the NaN slab fix.
//
// It exists to be wrong. RayDifferentialHarness evaluates every ray against this, against the live
// TriangleBvh, and against BruteForceOracle, so that a verdict which changed can be adjudicated
// rather than assumed. Do not edit this file to track the live one: the moment it stops carrying
// the old behaviour it stops proving anything.
//
// Nothing is relaxed for it: it builds under the same analyzer and warnings-as-errors regime as
// everything else, so keeping the old behaviour means keeping it while it still compiles clean.
...
/// </summary>
internal sealed class LegacyTriangleBvh
{
    ...
    private LegacyTriangleBvh(float[] v, int[] order, Node[] nodes, int triangleCount)
    {
        _v = v;
        _order = order;
        _nodes = nodes;
    ...
    public static LegacyTriangleBvh Build(float[] v, int count)
    {
        ...
        return new LegacyTriangleBvh(v, order, nodes.ToArray(), count);
    }
